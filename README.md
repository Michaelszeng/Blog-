...
<sup>1.</sup>########## i.e. Bode Plots to analyze frequency response of an oscillating system, Transfer Functions to model the system (what output it produces for a particular input), Root Locus Plots to analyze the impact of one parameter on a system, Nyquist Plots to determine if a system is stable.
