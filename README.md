...
where $c_t$ is the cost at time $t$, $e_t$ is the deviation from the reference path $(e_t = x_t - x^r^e^f_t)$, and $u_t$ is the acceleration and steering rate (a vector containing  acceleration and angular velocity). $Q$ and $R$ are matrices with constants that determine the weights of each of these factors.
